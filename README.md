...
Shared **frontend chrome** as files: HTML/Jinja templates, bundled CSS/JS, and
an HTTP contract. FastAPI + Jinja2 is one language binding (`app_factory[platform]`),
not the source of truth. Another language clones the git tag and reads the same
files.
...
**Tag:** `v0.7.7` (chrome contract in files; multi-user BOM is v0.7.7 / my-auth v0.5.6 / my-usermanager v0.6.7)
...
|-------|------|----------------------|
| **app-factory** (this repo) | Chrome files + HTTP contract; FastAPI binding is optional | `git` tag `v0.7.7` directly; multi-user hosts follow `COMPAT.md` |
...
| Concern | Owner |
|---------|--------|
| Bundled Basecoat UI / HTMX / Alpine files + manifest | **app-factory** `assets/` |
| Identity paths, HTMX redirect, Origin CSRF, Jinja subset | **app-factory** `contract/` |
| Shared `<head>`, shell, optional CDN pins, and theme boot | **app-factory** `templates/` |
| FastAPI mount / Jinja renderer / identity composer | **app-factory** `[platform]` binding |
...
| Module / path | Purpose |
|---------------|---------|
| `app_factory/contract/` | Language-neutral paths, HTTP rules, Jinja subset (`paths.toml`, `http.toml`) |
...
| `app_factory.conformance.assert_public_origin_csrf` | Probe public Origin against a different backend HTTP base URL (TLS terminator vs uvicorn) |
...
- Use `assert_public_origin_csrf(...)` instead of probing Origin CSRF against
  `http://testserver`. The public origin must differ from the backend base URL.
...
[tool.uv.sources]
app-factory = { git = "https://github.com/mikolaj92/app-factory.git", tag = "v0.7.7" }
...
assert_public_origin_csrf(app, public_origin=..., mutation_path=..., backend_base_url=...)
...
|-------------|---------------|-------|
| **v0.7.7** | **1.0.2** | Current: chrome contract in files (`contract/paths.toml`, `contract/http.toml`, templates). FastAPI is one binding. Same companions as v0.7.6. |
| **v0.7.6** | **1.0.2** | Same-origin `/favicon.ico` plus `login_redirect` (`?next=` and `HX-Redirect`). Same companions as v0.7.5. Prefer v0.7.7. |
| **v0.7.5** | **1.0.2** | `assert_public_origin_csrf` probes public Origin vs a different backend HTTP base URL. Same companions as v0.7.4. Prefer v0.7.7. |
| **v0.7.4** | **1.0.2** | `identity_authenticated_shell` loads `usermanager-ui.css` when `static_url_path` is set. Prefer v0.7.7. |
| **v0.7.3** | **1.0.2** | Companion my-usermanager `v0.6.7` (`install_local_identity`, durable OIDC flows, unlink-aware account UI). Chrome omitted `usermanager-ui.css`. Prefer v0.7.7. |
| **v0.7.2** | **1.0.2** | Invite default `/admin/users`; `install_manifest` exported; preferred BOM tags exist in git. Same Tailwind `@layer` contract as v0.7.1. Prefer v0.7.7. |
| **v0.7.1** | **1.0.2** | Tailwind virtual imports stay in `@layer`; same `create_product_app` / `RunPort` / `template_response` contract as v0.7.0. Prefer v0.7.7. |
| **v0.7.0** | **1.0.2** | Same-origin Tailwind browser engine instead of a CSS safelist. Prefer v0.7.7. |
| **v0.6.23** | **1.0.2** | Thin product host + opt-in run/intake views. Same my-auth v0.5.4 / my-usermanager v0.6.5. Prefer v0.7.7. |
