...
**Tag:** `v0.6.14` (identity-adapter composition; multi-user BOM is v0.6.14 / my-auth v0.4.7 / my-usermanager v0.5.10)
...
|-------|------|----------------------|
| **app-factory** (this repo) | Bundled chrome, one FastAPI mount, and the shared Jinja shell | `git` tag `v0.6.14` directly; multi-user hosts follow `COMPAT.md` |
| **basecoat-factory** | Maintainer-only build source for the generated Basecoat/UI asset bundle | Not a runtime dependency |
| **my-auth** (`fastapi-htmx`) | Generic passkey login/register UI | BOM tag `v0.4.7` |
| **my-usermanager** (`fastapi-htmx`) | Generic account/admin UI | BOM tag `v0.5.10` |
...
[tool.uv.sources]
app-factory = { git = "https://github.com/mikolaj92/app-factory.git", tag = "v0.6.14" }
my-auth = { git = "https://github.com/mikolaj92/my-auth.git", tag = "v0.4.7" }
my-usermanager = { git = "https://github.com/mikolaj92/my-usermanager.git", tag = "v0.5.10" }
```
...
|-------------|---------------|-------|
| **v0.6.14** | **1.0.2** | Additive on v0.6.11: `install_identity_adapters` + focused passkey/usermanager/session helpers; BOM row my-auth v0.4.7 / my-usermanager v0.5.10. No chrome change. |
| **v0.6.11** | **1.0.2** | Shared browser mechanisms (HTMX redirect, session CSRF, toast boot, pagination). Multi-user BOM stayed on v0.6.10 until matching auth tags; chrome generation with my-auth v0.4.7 / my-usermanager v0.5.7 had no composer. |
