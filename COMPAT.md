...
|-------------|---------|----------------|----------|
| **v0.6.15** | **v0.4.8** | **v0.5.11** | **Identity-adapter ownership cleanup** (same `install_identity_adapters` composer; UM uses canonical authenticated shell + platform session, split UI route groups, and split SQLite schema/store owners behind a stable facade). Hosts bump the full immutable row together |
