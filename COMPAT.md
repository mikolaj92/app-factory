...
|-------------|---------|----------------|----------|
| **v0.6.27** | **v0.5.1** | **v0.6.2** | **Neutral multi-user/multi-admin boundary**: authentication has no first-account bootstrap or enrollment policy; hosts own registration exposure and initial grants. `show_registration_link` and `registration_allowed` are removed from the adapter contract |
