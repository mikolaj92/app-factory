...
def install_app_factory_ui(
    ...
    *,
    ...
    installed = cast(AppFactoryUi | None, getattr(app.state, "app_factory_ui", None))
    ...
    configured = cast(
        set[int], getattr(app.state, "app_factory_jinja_environments", set[int]())
    )
    for environment in environments:
        if id(environment) in configured:
            continue
        _ = configure_jinja_env(environment)
        globals_dict = cast(dict[str, object], environment.globals)
        globals_dict["platform_asset_prefix"] = requested.asset_prefix
        globals_dict["platform_asset_url"] = cast(
            Callable[[str], str],
            partial(platform_asset_url, prefix=requested.asset_prefix),
        )
