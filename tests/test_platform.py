...
def test_build_platform_context_guest_vs_user() -> None:
    config = PlatformConfig(
        ...
        paths=PlatformPaths(
            ...
            invite="/admin/users/invite-legacy",
        ),
    ...
    assert paths.invite == "/admin/users/invite-legacy"
...
def test_platform_paths_defaults_match_adapter_contract() -> None:
    ...
    assert paths.invite == paths.admin_users == "/admin/users"
...
def test_identity_navigation_is_enabled_and_authorized_by_host_config() -> None:
    ...
    assert admin["platform_identity_menu"][3].active is True
