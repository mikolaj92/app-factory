...
| Package | Tag |
|---------|-----|
| app-factory | `v0.7.1` (editable path while developing this repo) |
| my-auth | `v0.5.6` |
| my-usermanager | `v0.6.6` |
...
(`passkey_enrollment_capabilities`) is stamped by my-usermanager v0.6.6
`initialize()` via my-auth v0.5.6 `ensure_sqlite_schema`, including already-current schemas.

Chrome generation v0.6.11 / v0.4.8 / v0.5.7 had no composer — bump
app-factory to v0.7.1 (and UM to v0.6.6 if still on v0.5.7).

## Why `override-dependencies`?

`my-auth` and `my-usermanager` each declare nested `tool.uv.sources` for older
app-factory tags (UM v0.6.6 / my-auth v0.5.6 nest an older app-factory tag). Without a host override, `uv lock` fails with conflicting
URLs. This example (and production hosts) force one app-factory source.
Do **not** override my-auth: UM v0.6.6 already nests my-auth@v0.5.6.
