...
| Package | Tag |
|---------|-----|
| app-factory | `v0.6.13` (editable path while developing this repo) |
| my-auth | `v0.4.7` |
| my-usermanager | `v0.5.9` |
...
(`passkey_enrollment_capabilities`) is stamped by my-usermanager v0.5.9
...
app-factory to v0.6.13 (and UM to v0.5.9 if still on v0.5.7).

## Why `override-dependencies`?

`my-auth` and `my-usermanager` each declare nested `tool.uv.sources` for older
app-factory tags (UM v0.5.9 / my-auth v0.4.7 nest app-factory@v0.6.11). Without a host override, `uv lock` fails with conflicting
URLs. This example (and production hosts) force one app-factory source.
Do **not** override my-auth: UM v0.5.9 already nests my-auth@v0.4.7.
